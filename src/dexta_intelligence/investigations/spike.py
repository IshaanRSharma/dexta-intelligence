...
WAVE5 §2: the tool path is *code*, not model choice. The orchestration runs
the §1 trace deterministically (orient → locate → zoom → carbs → boluses →
basal → IOB/COB → similar events); the LLM appears exactly once, at the end,
to phrase the headline from the gathered evidence — guard-audited, with the
deterministic headline as fallback. ``confidence`` is computed, never
self-assessed. ``limitations`` lists every step that could not run.
...
#: The frozen output contract (WAVE5 §2 / metric M10).
...
def explain_spike(
    ...
    *,
    ...
    """Explain the spike at/around ``when`` (ISO datetime, or ISO date to
    auto-locate the day's largest excursion). Returns the frozen output dict.
    Never raises on bad input — returns an explanation with limitations."""
    ...
    if model is not None and contributor_found:
        headline = _polish_headline(model, headline, evidence, pool)
...
def _inspect_treatments(
    ...
    """The §1 steps 4-6: carbs, boluses+IOB, basal — each skipped (and listed
...
def _attribute(  # noqa: PLR0911 — one return per allowed contributor pattern
    *,
    ...
    Returns ``(headline, contributor_found)``. The vocabulary is the WAVE5 §4
...
def _polish_headline(
    ...
    """One guarded LLM call to phrase the headline; deterministic fallback."""
    prompt = (
        "Rewrite this finding as one or two plain sentences for the user. "
        "Use ONLY numbers present in the evidence lines. Never give dosing, "
        "ratio, or timing directives — describe the pattern as a discussion "
    ...
    try:
        ...
        logger.warning("explain_spike: headline polish failed; keeping deterministic")
        return headline
    if not text:
        return headline
    ...
    if not report.ok:
        logger.warning(
            "explain_spike: polished headline failed the guard (%d violations); "
